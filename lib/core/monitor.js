...
class Monitor extends Searcher {
    ...
    async filterLotteryInfo() {
        ...
        await try_for_each(protoLotteryInfo, async function (lottery_info) {
            ...
            const
                ...
                needTopic = [...new Set(description.match(/(?<=[带加]上?(?:话题|tag).*)#.+?#|(?<=[带加])上?#.+?#\s*(?=话题|tag)/ig) || [])].join(' '),
            ...
            let isBlock2=false;
            if(blockword.length){
                for (let word of blockword) {
                    let regex = new RegExp(word);
                    let destext='';
                    if (reserve_lottery_text) {
                        destext = (description + reserve_lottery_text).replace(/\u200b/g, '');
                    } else {
                        destext = description.replace(/\u200b/g, '');
                    }
                    log.debug('匹配的文本值',destext);
                    if (regex.test(destext)) {
                        isBlock2 = true;
                        break;
                    }
                }
    ...
    async go(option) {
        ...
        let
            status = 0,
            { uid, dyid, chat_type, rid, relay_chat, ctrl, chat, uname  } = option,
            { check_if_duplicated, is_copy_chat, copy_blockword, is_repost_then_chat, is_not_create_partition } = config;

        const oldrelay_chat=relay_chat.split('//',2),
            istap=oldrelay_chat[0].includes('#'),
            ctrlObj=JSON.parse(ctrl);
        /* 评论 */
        if (rid && chat_type) {
            if (is_copy_chat) {
                const copy_chat = getRandomOne(
                    (await bili.getChat(rid, chat_type))
                      .filter(it => {
                           if (istap) {
                               return !(new RegExp(copy_blockword.join('|')).test(it[1])) && it[1].includes('#');
                           } else {
                               return !(new RegExp(copy_blockword.join('|')).test(it[1]));
                           }
                       })
                ) || [';;;;;;;;;', chat || '!!!'];
                chat = copy_chat[1]
                ...
                for(let i=0;i<(copy_chat[2].length>at_users.length ? at_users.length : copy_chat[2].length);i++){
                    if(uname && copy_chat[2][i].uname!==uname){
                        chat=chat.replace(copy_chat[2][i].uname , at_users[i][0] || '');
                    }
                }
                
                if (oldrelay_chat.length === 2){
                    relay_chat=chat+'//'+oldrelay_chat[1];
                    /**转发语修改了后，ctrl的@定位都需要修改*/
                    for(let item of ctrlObj) {
                        item.location = item.location+(chat.length-oldrelay_chat[0].length);  
                    }
                    ctrl=JSON.stringify(ctrlObj);
                }else {
                    relay_chat=chat;
                }
            } else {
