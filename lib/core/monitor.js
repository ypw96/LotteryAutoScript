...
class Monitor extends Searcher {
    ...
    constructor(lottery_param) {
        ...
        this.LotteryInfoMap = new Map([
            ...
            ['APIS2', this.getLotteryInfoByAPIS2.bind(this)]
        ]);
    }
    /**
     * 初始化
    ...
    async filterLotteryInfo() {
        ...
        await try_for_each(protoLotteryInfo, async function (lottery_info) {
            ...
            let
                ...
                needTopic = [...new Set(description.match(/(?<=(?:加|带|加入|参与)上?(?:话题|tag).*)#.+?#|(?<=(?:加|带|加入|参与))上?#.+?#\s*(?=话题|tag)/ig) || [])].join(' '),
                isRelayDynamic = type === 1,
                has_key_words = key_words.every(it => new RegExp(it).test(description)),
                isBlock = (function () {
                    if (!blockword.length) return false;
                    for (let word of blockword) {
                        let regex = new RegExp(word);
                        let destext = reserve_lottery_text
                            ? (description + reserve_lottery_text).replace(/\u200b/g, '')
                            : description.replace(/\u200b/g, '');
                        log.debug('匹配的文本值', destext);
                        if (regex.test(destext)) return true;
                    }
                    return false;
                })(),
                isLottery =
