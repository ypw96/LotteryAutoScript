...
function parseDynamicCard(dynamic_detail_card) {
    ...
    if (obj.type === 1) {
        ...
        /* 加码者的动态的ID !!!!此为大数需使用字符串值,不然JSON.parse()会有丢失精度 */
        obj.pre_dy_id =desc.pre_dy_id_str;
        /* 预约抽奖信息 */
        if (add_on_card_info.length > 0) {
            const [status, oid_str, text] = add_on_card_info
                .filter(it => typeof it.reserve_attach_card !== 'undefined'
...
class Searcher {
    ...
    static async checkAllDynamic(hostuid, pages, time = 0, offset = '0') {
        ...
        for (let i = 0; i < pages; i++) {
            ...
            const OneDynamicInfo = await hadUidGetOneDynamicInfoByUID(offset)
            const mDRdata = (() => {
                if (OneDynamicInfo) {
                  return modifyDynamicRes(OneDynamicInfo);
                } else {
                  return null;
                }
              })();
    ...
    async getLotteryInfoByUID(UID) {
        ...
        const fomatdata = await allModifyDynamicResArray
            ...
            .reduce(async (pre, cur) => {
                ...
                let is_pre_dy=false;
                if(cur.origin_dynamic_id!=cur.pre_dy_id){
                    is_pre_dy=true
                }
                let cardinfo;
                const dynamicId = is_pre_dy ? cur.pre_dy_id : origin_dynamic_id;

                if(is_pre_dy){
                    const card2 = await bili.getOneDynamicByDyid(dynamicId);
                    if(card2){
                        cardinfo=parseDynamicCard(card2)
                    }
                    await utils.delay(get_dynamic_detail_wait);
                }
        
                if (!check_if_duplicated || check_if_duplicated >= 2) {
                    const card = await bili.getOneDynamicByDyid(dynamicId)
                    log.info('获取动态', `查看源动态(${dynamicId})是否点赞 (${length--})`)
                    if (card) {
                ...
                results.push({
                    ...
                    create_time: is_pre_dy ? cardinfo.create_time : cur.origin_create_time,
                    is_liked,
                    uids: is_pre_dy ? [cur.uid, cardinfo.uid] : [cur.uid, cur.origin_uid],
                    uname: is_pre_dy ? cardinfo.uname : cur.origin_uname,
                    ctrl: [],
                    dyid: is_pre_dy ? cur.pre_dy_id : cur.origin_dynamic_id,
                    ...
                    rid: is_pre_dy ? cardinfo.rid_str : cur.origin_rid_str,
                    chat_type: is_pre_dy ? cardinfo.chat_type : cur.origin_chat_type,
                    des: is_pre_dy ? cardinfo.description : cur.origin_description,
                    type: is_pre_dy ? cardinfo.type : cur.origin_type,
