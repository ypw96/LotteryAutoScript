        ...
        const fomatdata = await allModifyDynamicResArray
            ...
            .reduce(async (pre, cur) => {
                ...
                results.push({
                    ...
                    create_time: cardinfo ? cardinfo.create_time : cur.origin_create_time,
                    is_liked,
                    uids: cardinfo ? [cur.uid, cardinfo.uid] : [cur.uid, cur.origin_uid],
                    uname: cardinfo ? cardinfo.uname : cur.origin_uname,
                    ctrl: [],
                    dyid: cardinfo ? cur.pre_dy_id : cur.origin_dynamic_id,
                    ...
                    rid: cardinfo ? cardinfo.rid_str : cur.origin_rid_str,
                    chat_type: cardinfo ? cardinfo.chat_type : cur.origin_chat_type,
                    des: cardinfo ? cardinfo.description : cur.origin_description,
                    type: cardinfo ? cardinfo.type : cur.origin_type,
    ...
    async getLotteryInfoByArticle(key_words) {
        ...
        for (const { id, pub_time } of cvs) {
            ...
            for (const dyid of dyid_set) {
                ...
                // 先检索本地dyids文件，跳过已转发过的，避免无谓的网络请求
                ...
                if (card) {
                    ...
                    // 仅剩"点赞判断"需要兜底(check_if_duplicated 为 2 或 3 时)
