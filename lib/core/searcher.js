...
class Searcher {
        ...
        for (const { id, pub_time } of cvs) {
            ...
            for (const dyid of dyid_set) {
                ...
                if (card) {
                    ...
                    if ((!check_if_duplicated || check_if_duplicated >= 2) && is_liked) {
                        log.info('获取动态', `动态(${dyid})已转发过`);
                        _weight += 1;
                    } else {
    ...
    /**
     * 从lottery_dyids目录下lottery_info_all_future格式JSON获取抽奖信息
     * (会将字段转换为标准LotteryInfo格式，uids永远二元、ctrl location换算、缺失字段补默认)
     * @param {string} filename
     * @returns {Promise<LotteryInfo[] | null>}
     */
    async getLotteryInfoByAPIS2(filename) {
        if (!filename) {
            log.warn('获取动态', 'APIS2文件名为空');
            return null;
        }
        // 去掉 file:// 前缀（和 APIs 的 getLotteryInfoByAPI 行为一致）
        const actualFilename = filename.startsWith('file://') ? filename.substring(7) : filename;
        log.info('获取动态', `开始获取文件(lottery_dyids/${actualFilename})中的抽奖信息`);

        const rawList = await utils.readLotteryInfoFile2(actualFilename);
        if (!rawList || !rawList.length) {
            log.warn('获取动态', `文件(${actualFilename})为空或不存在，已跳过`);
            return null;
        }

        const lottery_info = await rawList.reduce(async (pre, cur) => {
            const results = await pre;
            const { dyid } = cur;
            if (!dyid) return results;

            // 重复转发检查（和APIs的逻辑保持一致：≥2才网络查 is_liked）
            if (!check_if_duplicated || check_if_duplicated >= 2) {
                log.info('获取动态', `查看动态(${dyid})是否点赞`);
                const card = await bili.getOneDynamicByDyid(dyid);
                if (card) {
                    await utils.delay(get_dynamic_detail_wait);
                    const { is_liked } = parseDynamicCard(card);
            ...
            // —— 字段标准化转换 ——
            const author = Number(cur.author_mid || 0);
            const mentionArr = Array.isArray(cur.mention_mids) ? cur.mention_mids : [];
            // 取 mention_mids 去掉 author_mid 后的第一个（uids[1]，关注第二人）
            const extra = mentionArr
                .map(m => Number(m))
                .filter(m => m && m !== author)
                .slice(0, 1);

            // ctrl.location 直接用原始值（B站API实际按Code Point索引解释，不需要转换）
            const ctrl = Array.isArray(cur.ctrl)
                ? cur.ctrl.map(c => ({ ...c }))
                : [];

            const des = cur.des || '';

            // participation_type -> type 映射（加码type=1，非加码type=2）
            const type = cur.participation_type === 1 ? 1 : 2;

            /** @type {import('./searcher').LotteryInfo} */
            const standard = {
                lottery_info_type: 'apis2',
                create_time: cur.create_time,
                is_liked: !!cur.is_liked,
                // 永远二元：[author, 非本人mention第一位或null]
                uids: [author, extra[0] ?? null],
                uname: cur.uname,
                ctrl,
                dyid,
                reserve_id: cur.reserve_id ?? 0,
                reserve_lottery_text: cur.reserve_lottery_text ?? '信息丢失',
                is_charge_lottery: cur.is_charge_lottery ?? false,
                rid: cur.rid,
                chat_type: cur.chat_type,
                des,
                type,
                hasOfficialLottery: cur.hasOfficialLottery ?? false
            };
