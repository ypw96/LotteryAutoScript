...
async function createRandomDynamic(num) {
    if (config.create_dy) {
        ...
        if (allModifyDynamicResArray && allModifyDynamicResArray.length > 0) {
            const first12Elements = allModifyDynamicResArray.slice(0, num > 20? 20 : num);

            // 遍历每个元素并进行判断
            for (const element of first12Elements) {
                const { type, origin_type } = element;
                if ((type === 1 && origin_type === 8) || type === 2 || type === 4) {
                    cont++;
                }
            }
        }else{
            log.info('随机动态', '未找到用户动态，可能风控了');
        }

        if (num-cont>0) {
            log.info('随机动态', `已有${cont}条，准备创建${num-cont}条随机动态`);
            await randomDynamic(num-cont);
        } else {
