    ...
    if (clear_quick_remove_attention) {
        ...
        for (const page of infiniteNumber()) {
            ...
            for (const [index, dyinfo] of allModifyDynamicResArray.entries()) {
                ...
                const { type, dynamic_id, create_time } = dyinfo || {};
                const origin_uid = dyinfo?.origin?.uid;
                if (typeof type !== 'undefined'
                    ...
                    if (days_ago > clear_max_day) {
                        ...
                        if (clear_remove_attention) {
                            const cand_uids = new Set();
                            // 1. 沿整条转发链递归收集所有层级的 origin.uid
                            //    (parseDynamicCard 会递归嵌套生成 origin.origin，所以这里一直钻到最底)
                            let cur_origin = dyinfo?.origin;
                            while (cur_origin && cur_origin.uid) {
                                if (cur_origin.uid !== MY_UID) cand_uids.add(cur_origin.uid);
                                cur_origin = cur_origin.origin;
                            }
                            // 2. 补充 ctrl 里 @提到的人 (加码动态 //@ 文本里的作者可能不在转发链里)
                            if (dyinfo.ctrl && dyinfo.ctrl.length) {
                                dyinfo.ctrl.forEach(c => {
                                    if (c && c.type === 1 && c.data) {
                                        const at_uid = Number(c.data);
                                        if (at_uid && at_uid !== MY_UID) cand_uids.add(at_uid);
                                    }
                                });
                            }
                            for (const uid of cand_uids) {
                                if (before_separate.indexOf(uid) === -1
                                    && uid_list.indexOf(uid) > -1) {
                                    log.info('清理关注', `取消关注 uid=${uid}`);
                                    success = await retryfn(3, [false], () => bili.cancelAttention(uid));
                                    if (!success) break;
                                }
                            }
                        }
                        ...
                        if (origin_uid) {
                            ...
                            if(dyinfo.ctrl && dyinfo.ctrl.length){
                                dyinfo.ctrl.forEach(c => {
                                    if(c && c.data){
                                        const at_uid = Number(c.data);
                                        if (at_uid && before_separate.indexOf(at_uid) === -1) {
                                            before_separate.push(at_uid);
                                        }
                                    }
                                });
                            }
