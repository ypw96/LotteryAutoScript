...
const utils = {
    ...
    * infiniteNumber() {
    ...
    /**
     * 读取lottery_dyids目录下lottery_info_all_future格式的抽奖信息
     * 数据结构: { "lottery": [ {...}, {...} ] }
     * 目录不存在 / 文件不存在 / 格式错误 都不会报错，直接返回空数组
     * @param {string} filename  lottery_dyids目录下的文件名
     * @return {Promise<Array>}  解析出的 lottery 原始数组（未标准化）
     */
    readLotteryInfoFile2(filename) {
        return new Promise((resolve) => {
            try {
                if (!fs.existsSync(utils.lottery_dyids)) {
                    resolve([]);
                    return;
                }
            } catch (_) {
            ...
            fs.readFile(path.join(utils.lottery_dyids, filename), (err, data) => {
                if (err) {
                    ...
                    try {
                        const json = utils.strToJson(data.toString('utf8'));
                        if (json && Array.isArray(json.lottery)) {
                            resolve(json.lottery);
                        } else {
                            resolve([]);
                        }
                    } catch (_) {
