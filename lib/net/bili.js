    ...
    async fetch_session_msgs(talker_id, size) {
        ...
        if (res.code === 0) {
            ...
            if (msgs instanceof Array) {
                ...
                return msgs.filter(it => ![8].includes(it.msg_source)).map(it => { try { const c = JSON.parse(it.content); return c.imageType ? '消息图片' : c.content || ''; } catch(e) { return ''; } }).filter(Boolean).join('\n');
            } else {
