...
function wxPusherNotify(text, desp){
    return new Promise(resolve => {
        if (WXPUSHER_TOKEN) {
            const WXPUSHER_TOKEN_AY = WXPUSHER_TOKEN.split(',');
            send({
                method: 'POST',
                url: 'https://wxpusher.zjiecode.com/api/send/message',
                contents: {
                    appToken: WXPUSHER_TOKEN_AY[0],
                    summary: text,
                    content: desp,
                    contentType:1,
                    topicIds:[WXPUSHER_TOKEN_AY[1]],
                    verifyPay:false,
                    verifyPayType:0,
                    uids:[]
                },
                ...
                success: res => {
                    try {
                        ...
                        if (data.code === 1000) {
                            log.info('发送通知', 'WxPusher微信推送消息成功');
                        } else {
                            log.error('发送通知', `WxPusher微信推送消息异常\n${JSON.stringify(data)}`);
                        }
                    } catch (error) {
                ...
                failure: err => {
                    log.error('发送通知', 'WxPusher微信推送调用API失败！！' + err);
                    resolve();
                }
            });
        } else {
            log.debug('发送通知', 'WxPusher微信推送的应用TOKEN，取消微信推送消息通知');
            resolve();
        }
    });
}
...
function qywxAmNotify(text, desp) {
    return new Promise(resolve => {
        if (QYWX_AM) {
            ...
            // 有第5个参数 thumb_media_id 时用图文消息(mpnews)，否则用纯文本(text)
            const useMpnews = !!QYWX_AM_AY[4];
            const despHtml = useMpnews ? desp.replace(/\n/g, '<br>') : desp;
            send({
                method: 'POST',
                ...
                success: res => {
                    try {
                        ...
                        const messageContent = useMpnews
                            ? {
                                msgtype: 'mpnews',
                                mpnews: {
                                    articles: [
